...
export function status(vault: Vault, daemonUrl: string): void {
  ...
  console.log(`app        ${daemonUrl} (start it with: cookiejar ui)`);
}
...
/** Mirrors ticking cookies in the app: pick a site, optionally narrow to names. */
...
export function newGrant(
  ...
  console.log('This is the only time the token is shown. Tokens only work while the jar is unlocked.');
...
/** The terminal twin of the app's "Give it to an agent" panel. */
export function share(vault: Vault, bundleId: string, opts: { tunnel?: string; port: number }): void {
  ...
  console.log(`\nAgent in the cloud — the daemon is loopback only, so tunnel it:\n`);
